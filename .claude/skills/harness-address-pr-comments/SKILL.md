...
- **4d return:** one preamble block (PR/branch/author/url/repo/review-status/linked-issues/scope/files/total/counts/**`RUN_N` + `ROUND_RANGES`** — Phase 5 renders from returned data and must not re-fetch, so an unreturned `RUN_N` means the 5c.1 notice silently never fires, and a missing `ROUND_RANGES` silently disables 6b.2b's round-inversion lens)
