...
> **Judgment-pass reviewer (hard dependency):** 6b.2b's judgment pass runs as `harness:review-change`
> `build-run` — the one review mechanism, isolated (doer ≠ judge). Installed alongside like OpenSpec;
> absent → stop and tell the operator to install `harness:review-change` (Phase 1.5 step 1 aborts on
> it), never grade your own fix diff inline.
...
4. **Stop only at genuine forks — no plan-approval gate.** After analysis, walk DECISION-NEEDED forks
   ...
   DECISION-NEEDED finding (5d), a mid-flight cascading Decision-Gate hit (6b.1), a round-inversion
   finding (6b.2b lens, walked via 6b.1), an unrecognized dirty path at 6b.2 (aborts), or corrective
...
```
**Contract handoff (analysis spawns).** Main agent resolves
`../walk-me-through/references/walk-me-through.md` from this skill's injected base dir and hands the
**absolute** path to the Phases 1–4.5 sub-agent as `Fork-card contract: <abs path>` — the same
literal key the nested Phase-4 prompts carry, so the whole chain greps as one; that sub-agent passes
the same absolute string on to every Phase-4 nested sub-agent. A sub-agent has no base dir of its
own, so an unhanded contract can only be improvised or skipped.
...
3. **No page-cap saturation.** Result count **equal to** the requested page size on a read that did
   NOT exhaust pagination (rule 2) → `unknown`, re-read with pagination; a read proven exhausted per
   rule 2 is complete regardless of count. (A bare `reviews(first:60)` returning exactly 60 is
   truncation, not sixty.)
...
`fixed:` replies by **any author with real write permission** — `gh api repos/$OWNER/$NAME/collaborators/<login>/permission --jq '.permission'` ∈ `admin`/`write` (**not** `author_association`: that's a social label, `MEMBER` only means org member and both it and `COLLABORATOR` can carry read-or-triage-only access, so it authenticates nothing) — **and whose cited sha is an ancestor of the PR branch**. One call per distinct marker author, cached for the run — the trailer alone is unauthenticated text, so an outside commenter could otherwise post two fake `commit:<sha7>` trailers and inflate the 5c.1 notice, seed 6b.2b's round-inversion lens with foreign shas, or spoof `region_map` into false repeat-patch forks. Write-access scoping keeps the team case (teammates run it from different accounts) while excluding drive-by comments; only `fixed:` — an `already:` sha or the literal `pre-existing` isn't a patch by this
...
reads 1. This run's patch-round ordinal on the PR (feeds the 5c.1 convergence notice). **Keep each round's commit RANGE — `ROUND_RANGES`**: **one entry per distinct `commit:` sha** — the same key `RUN_N` counts on; every reply of a round cites the same final sha, so keying on it yields one range per round, not one per thread. Value = that round's **`range:<base7>..<final7>`** when present (6e emits it for a multi-commit round) — **both endpoints must resolve and be ancestors of the PR branch**, the same authentication the cited `commit:` sha gets: an unresolvable endpoint errors the lens's `git diff` and a foreign one seeds it with foreign hunks. **Either check failing, or no `range:` at all → fall back to `<sha>^..<sha>` on the authenticated `commit:` sha** — never drop the round (dropping loses the whole round; the `commit:` sha is authenticated, so the fallback can't seed foreign hunks). **Whether that fallback is exact or narrow turns on WHY `range:` is missing — mark `endpoint-only` only in the narrow case:** *absent* `range:` → per 6e a round emits it whenever it lands >1 commit, so absence means one commit and `<sha>^..<sha>` is **exact** (`$START_SHA` is that commit's parent) — **no mark, no report line**, else every ordinary round warns and the signal is worthless. *Present but an endpoint fails* → the round announced itself multi-commit and its base is unusable, so `<sha>` — the round's FINAL commit — narrows the lens to that last delta: precisely the blindness `range:` exists to close, back for that round alone → **mark it, and 6b.2b names it in the report**; a narrowed lens that reads clean is indistinguishable from a clean one. **Known bounded gap:** a reply written before `range:` existed is indistinguishable from a one-commit round, so a legacy multi-commit round stays silently endpoint-only — it ages out as new rounds land; don't warn on every round to cover it. 6b.2b's round-inversion lens diffs against these — a round can land >1 commit (6c corrective re-entries). The `RUN_N` count itself stays over distinct `commit:` values — unchanged, one per round.
...
**Every fan-out prompt carries** `Fork-card contract: <the absolute path handed to this sub-agent>` —
read it for option admissibility. 4c derives `Admissible options` (and 4.5 a `gate_deferred`
candidate's) **gated through that contract › Admissibility**; 5d renders what arrives and adds
nothing, so an unhanded contract ships banned or filler rows straight into the card.
...
- **4d return:** one preamble block (PR/branch/author/url/repo/review-status/linked-issues/scope/files/total/counts/**`RUN_N` + `ROUND_RANGES`** + **`standards_summary` + `VERIFY_CMDS`** — Phase 5 renders from returned data and must not re-fetch, so an unreturned `RUN_N` means the 5c.1 notice silently never fires, and a missing `ROUND_RANGES` silently disables 6b.2b's round-inversion lens; Phase 3 runs **inside this sub-agent**, so unreturned `standards_summary`/`VERIFY_CMDS` leave the main agent nothing to hand 6a and 6b.2b and nothing to run at 6b)
  ...
  **not the context span** (that would swallow the hunk's context and false-fire when a later fix touches
  ...
  + per-thread block — **skipped threads use this same shape** so 6e.1 can join them — (`#`, `ThreadID`, `RootCommentID`, **`skip_reason`** when skipped — its prior tag rides `Reply tag` — **`ReviewIDs`** = the set of `pull_request_review_id` across the thread's 2a comments; *every* review with a comment in the thread, since a later review can reply into an earlier one's thread and the verdict belongs to both, `File L<line>`, `Reviewer`, `Summary`, `Verdict`, `Gate`, `Reasoning` citing standards, `Fix plan`/**`Admissible options`** (the **complete** list 4c derived — one per line as `<name> | <terse pro> | <terse con> | <executable outcome — its class first, then content: code fix / recorded write → files + exact change · follow-up issue (6g) · reply-only, no write>`, however many there are — Phase 6a receives only this payload, so an option without its own executable outcome cannot be implemented when the operator picks it over the recommendation. **The first three fields ARE the 5d card's row, verbatim** — they fill the card's `Option | Pros | Cons` columns (the *card's* own `#` column carries the letter — not this block's `#`, which is the thread number). **The executable outcome is NOT a rendered column** — implementation payload carried *alongside* the row for 6a to apply on a pick, **never printed in the card**; rendering all four spills the write plan into the Cons column. **Never an `Option A`/`Option B` pair** — a fixed two-slot shape silently discards a third resolution, and Phase 5 cannot re-fetch to recover it)/**`Recommended option`** (which, plus the concrete signal) / **`Cost if recommended`**/`Blocker`, `Reply tag`, `Code context`).
...
- **Emit** per swept class: `{class, source_thread, tier1_siblings:[file:line], tier2_instances:[file:line],
  gate_deferred:[file:line]}` → tier1 → 6a; tier2 + gate_deferred → report. **A gate_deferred candidate
  gets a full 4c derivation before 5d renders it** — admissible options (each with its executable
  outcome), recommended option, cost, blocker — synthesized for the CANDIDATE (its file:line, its
  surface), not inherited from the AUTO-FIX source thread, which cannot supply candidate-specific
  alternatives. Bare `file:line` never reaches the wizard.
...
- **5c.1 convergence notice (after the 5a–5c tables, before 5d/Phase 6):** `RUN_N ≥ 3` **and** (A>0 or
  D>0) → emit `⚠️ run #<RUN_N> on this PR — patch rounds not converging; a fix reverting an earlier
  round's stops at 6b.2b round-inversion` as a **non-blocking notice** (repeat it in the Phase-7
  report), then proceed — **no 👉 gate**. A bare round count carries no decision (the only rational
  answer was always "proceed"; measured on kino #246: the count alone would have gated every round
  while naming nothing); the blocking stop for real non-convergence is the **round-inversion** lens
  (6b.2b), which fires on the contradiction itself and names it. (A no-op rerun — zero auto-fix, zero
  decisions — emits nothing; 6c.1 short-circuits it anyway.)
- **Option-pick format:** render a walk-me-through fork card (`../walk-me-through/references/walk-me-through.md`) — `Q<N> of <total>` + `#<N>` title, framing (comment / why-it-needs-a-decision), options table (terse Pros/Cons), grounded Recommendation (pick + reasoning + `Cost if`), `Escape:` + `Pick:` lines; operator replies by letter. **Never `AskUserQuestion` or a native picker.** One fork per turn. Yes/no gates one line.
- **5d wizard (DECISION-NEEDED only):** zero → skip, "No forks — proceeding." For each, in order: render the card (decision #, file:line, comment quote, code context, which gate criterion, options table, Recommendation, plus `Escape:`/`Pick:` lines); operator replies by letter; never `AskUserQuestion`. **Rows = the thread's `Admissible options` list, its first three fields verbatim (`Option | Pros | Cons`; the 4th — the executable outcome — is 6a's payload, never a rendered column), in the order 4c derived it, lettered consecutively from `A` — never a fixed `A/B/C/D` role mapping** (a fixed mapping drops a third live fix and leaves gaps when a role is absent). Each row already passed `../walk-me-through/references/walk-me-through.md` › Admissibility, so render what arrived and add nothing: no filler row, and `Decline finding` / `Defer (blocked)` appear **only** when 4c admitted them (declining defensible on the merits · deferral under a concrete blocker or as a recorded terminal disposition — never for "out of scope" or "big change"; correctness over scope). Escape = the next free letter.
  **Exactly ONE admissible option → consent gate, not a card.** A criterion can fire (public contract ·
  ...
  <blast radius>. Apply?` per `../walk-me-through/references/walk-me-through.md` › one-option consent gate. Yes → the item
  joins the 6a batch and records `{path, pending}` in `GATE_DECIDED`. **No → the thread stays OPEN** —
  denial rejects the repair, not the finding (a load-bearing finding is not proven wrong by "not now").
  Reply `unclear: operator declined the repair (<one-line what>); finding stands — how should it be
  addressed?`, do not resolve, count it unaddressed in the report. Never a one-row table, never a padded
  second row.
...
Runs after the 5d wizard, or immediately if no forks. Invocation is consent; no per-step re-confirm. Stop only for a mid-flight cascading decision (6b.1 — incl. a 6b.2b round-inversion finding), an unrecognized dirty path (6b.2), corrective re-entries exhausted (6c), or a hard-gate failure.
...
- **6a implement (sub-agent fan-out default):** build the batch graph (independent → parallel, dependent → sequential; same-file grouped; structural items single-threaded); dispatch one sub-agent per independent batch in a single message. Each sub-agent gets its items + fix plans, the scope statement, the Phase-3 standards summary, **the Decision Gate criteria verbatim** (a fresh sub-agent doesn't inherit this skill's body, and 6b.1 makes it sort its own cascading findings AUTO-FIX-vs-gate — unhanded, it silently auto-fixes a public-contract or schema hit), and `VERIFY_CMDS`; implements, verifies its batch, returns `{batch_id, files_touched, artifacts_observed, tracked_observed, verify_status, errors, cascading_findings}` — **`cascading_findings` are findings-only — a batch agent never derives admissible options, so this spawn is handed no fork-card contract** (the explicit findings-only marker every spawn site must carry): it reports what it hit as **`[{path, line, what, gate_criterion|none, evidence, intended_fix}]`** — shaped, because 6b.1 renders a card from it and a bare summary cannot fill one (the same gap `design_stops` closes on the 6b.2b path); 6b.1 on the main agent derives the options, re-reading `path` when 4c never mapped that file. The union of `files_touched` seeds `FIX_SET`; `artifacts_observed` (per 6b's snapshot rule) is evidence about **untracked** paths only — it decides *which ask* 6b.2 raises, never that a path may be deleted, and never ownership of a **tracked** path. **Keep on main agent (don't fan out)** when: ≤3 mechanical items; any item touches load-bearing shared config (serialize); operator chose Other with no concrete plan. **Fold in Phase-4.5 tier-1 swept siblings** — each rides its owning finding's batch; the implementer confirms every candidate genuinely matches the class before fixing (per 4.5), skipping any that don't. **Always update tests inline** with each behavioral change.
...
- **6b.1 cascading-finding policy** (something found during the fix loop, not in the comments): AUTO-FIX class → fix silently in the batch, track for the report; Decision-Gate hit → stop the batch, mid-execution walk-me-through fork card — **the main agent derives the options here**, not the batch agent that surfaced the finding (it holds the base dir, the Phase-3 standards and the 4c fix plans, and unlike Phase 5 may re-read) — **scoped to a 6a-batch-surfaced finding**: a 6b.2b `design_stops` block arrives with its options already derived by the contract-handed reviewer, so render it verbatim and never re-derive — (same admissibility-gated derivation as 5d — the fix + any genuine alternative; **exactly one admissible option → 5d's one-option consent gate, never a one-row card** — a 6b.2b `design_stops` block legally carries one when a must-stop rule forced the stop; `Decline` / `Defer (blocked)` only under their 5d carve-outs; then `Escape:`/`Pick:`), resume after; genuinely blocked → stop batch, file a follow-up, `deferred:` reply, continue other batches. Never silently expand beyond AUTO-FIX class. Any file a cascading fix touches → `FIX_SET`; a Decision-Gate pick here records **`{path, pending}`** in `GATE_DECIDED`, as 5d does — this fork runs before 6b.2 stages, so `git rev-parse :<path>` would read the pre-implementation entry (or fail outright for a new path) and b1 would re-ask the operator the same question.
...
- **6b.2b certify** — runs after reconciliation on every path that continues (the certification lives here, not inside 6b.2's STOP bullet; a run that took STOP has aborted and never reaches it): `git diff --cached $START_SHA --stat` + `git diff
  --cached $START_SHA`; certification = **the mechanical lenses (inline, below) + the isolated judgment pass (end of this
  bullet)**, both over the full diff — added lines **and** deletions/modification pairs.
  ...
  **Ambiguous** (two threads' items in one file, can't tell which produced the line) → **do not exclude,
  ...
  which 1.5 step 4 guarantees equals `$START_SHA` by refusing to run ahead of origin. Per hunk
  ...
  line a prior round added** — for each range in `ROUND_RANGES`, over each staged path both touch:
  `git diff <range> -- <path>` (**a range, not `git show <final-sha>`**: a round can land >1 commit —
  6c hook-fail + corrective re-entries — and the endpoint's delta alone misses an earlier commit's
  lines, so an inversion of them passes unnoticed). **A round Phase 2 marked `endpoint-only`** (it
  announced >1 commit but its `range:` failed authentication) **carries exactly that gap — emit
  `round-inversion narrowed — <N> round(s) endpoint-only: <sha7>[, <sha7>…]` in the report**, never let
  the narrowed pass read as clean coverage; a **candidate** = a whitespace-normalized `+` line of the prior round
  ...
  <path>` — with `Decline finding — reply citing the contradiction` admissible alongside the fix
  ...
  **Mechanical lenses run inline — as greps and diff walks, not as judgment**:
  ...
  round-inversion is `git diff <range>` + normalized compare + neighbour/range correlation. A
  ...
  **The judgment pass is NOT inline — spawn `harness:review-change` `build-run` scoped to this staged
  ...
  authored a fix must not grade it (measured: kino #246 — 7 fix-created findings walked past the
  author-run check into the next bot round; harness-pipeline #45 — 4 such rounds). Hand the sub-agent
  the Phase-3 standards summary + the 4c fix plans (no build artifacts exist on this path) and two
  focus hints: **new surface** (fresh null/bounds gap, type hole, dead code, over-claiming comment/doc
  phrase, lint/complexity ceiling just crossed) · **lost surface** (a deletion that removes a
  guard/validation/behavior with no replacement on the added side). The reviewer-fixer fixes clear
  no-trade-off findings itself — its `files_touched` → `FIX_SET` — and returns `judge_findings`:
  fold them under the same 2-pass cap below; **`design-stop` ones → 6b.1, rendered from the returned
  `design_stops` blocks** (`build-run` has no wizard, so a `decision-needing` finding there is either
  auto-refuted or escalated to `design-stop` — `queued` never survives; and the `judge_findings` triple
  `{summary, category, disposition}` cannot fill a fork card — no `File L<line>`, no `Admissible
  options`, no `Recommended option`, no `Cost if recommended`). Artifact write only when
  ...
  successful run. **No inline fallback** — `harness:review-change` is a hard dependency gated at 1.5
  step 1, so a run never reaches 6b.2b without it; never grade your own fix diff inline, never skip
  the pass. **Emit findings only** (one `file:line — <finding>` each; no per-check "clean"
...
- **6e reply in-thread (machine-readable)** — **`PUSH_OK` gates only tags citing this run's commit**
  ...
  posts regardless. Tags — `fixed: <what>. commit:<sha7>` (when Phase-4.5 tier-1 siblings were fixed under this thread, append ` swept:<N> same class` before `commit:` — tells the reviewer/bot the class was cleared; add up to 2 file **basenames** only if the fully-serialized body incl. trailer stays ≤200, else emit the count alone — the report's Class-sweep section carries the full file list) · DECISION-NEEDED `fixed: <what>. choice:<letter|custom>. commit:<sha7>` (`letter` = the picked 5d row's actual letter — rows letter dynamically, never a fixed set) · `wontfix: <reason>. ref:<path/rule>` · `already: <where>. commit:<sha7|pre-existing>` · `unclear: <question>` · `deferred: <issue-url>`. **For `fixed:` tags (incl. the DECISION-NEEDED form), `<sha7>` is always the run's FINAL `$COMMITTED_SHA`** (the one the remote has), never a per-thread landing commit — a multi-commit run otherwise seeds two distinct `commit:` values and inflates the next run's `RUN_N` into a false brake. **Run landed >1 commit → append ` range:<base7>..<final7>` right after `commit:<sha7>`** (`fixed: <what>. commit:<sha7> range:<base7>..<final7>`), `base` = **`$START_SHA`** — the commit the round started FROM, **never the round's first commit**: `git diff <first>..<final>` excludes `<first>`'s own delta entirely (on a 2-commit round it collapses to `<final>`'s delta alone — `git show <final>`'s diff body), so a first-commit base drops the very commit it names — the exact miss this field exists to close. Same `<base>..<tip>` shape as the one-commit fallback. `commit:` still cites the final sha (one `commit:` per round keeps `RUN_N` honest) and `fixed:` is still the first token; `range:` is what the next run reads into `ROUND_RANGES` so 6b.2b's round-inversion lens sees every commit of the round, not just the endpoint's delta. One commit → no `range:` (Phase 2 derives `<sha>^..<sha>` — the same base, since `$START_SHA` is that commit's parent). **`already:` is exempt**: it cites the commit that actually made it true — **verified present on the remote** (`git merge-base --is-ancestor <sha> origin/<branch>`), else emit `pre-existing` rather than a sha a reviewer cannot find — this run may have made no commit at all. No greetings/thanks/backticks; ASCII; ≤200 chars (hard cap 500 excl. trailer); tag is first token (parsers split on `:`). **Validate the serialized body length (incl. trailer) before the API call** — over 200 → drop the `swept` file list first, then truncate `<what>`; **never drop `commit:` or `range:`** (the next run reads them for `RUN_N` and `ROUND_RANGES`); never exceed the 500 hard cap. **Mandatory signature trailer** — blank line then `[harness:address-pr-comments]` on its own final line (idempotency). Post: inline reply `gh api repos/$OWNER/$NAME/pulls/$PR/comments/$ROOT_COMMENT_ID/replies -f body="$(printf '%s\n\n[harness:address-pr-comments]\n' "$BODY")"` (use `-f body=`, not `--input -`); top-level review/issue → issue comment with a parseable `Re-review-<review-id>:` header line + the tagged reply. Throttle `sleep 2`; on 422 abuse / 403 Retry-After honor header or wait 60s, retry. >20 replies → aliased GraphQL mutations in **batches of ≤10 aliases per request** (6e.2).
...
- **6e.2 bulk reply (N>20):** aliased GraphQL mutations (`r1: addPullRequestReviewThreadReply(...)`, `r2: ...`) in **batches of ≤10 aliases per request** — **never one request for all**: GitHub's per-request mutation budget returned `RESOURCE_LIMITS_EXCEEDED` from the 12th alias on (measured, harness #51: aliases 1–11 posted, 12–22 failed in the same response), so a single 22-alias call half-posts and a naive retry double-posts the first half. **Before any retry, re-read the thread's replies and post only the aliases with no trailered reply.** REST fallback ≤20 with throttle.
...
self-check own fix diff before commit (6b.2b: mechanical lenses inline, judgment via isolated `review-change build-run`) · convergence notice at run #3 (5c.1) · round-inversion stops a repeat round (6b.2b) ·
