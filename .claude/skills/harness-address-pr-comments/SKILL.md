...
- **6b.2 fix-diff self-check (inline, no sub-agent) — 6c won't commit without it:** stage exactly
  ...
  **cross-batch** (two 6a sub-agents on one surface) · **region** (this run touched a line **inside any
  ...
  Findings → fix, re-stage, re-run 6b + 6b.2, commit once; **cap 2 passes** — a pass-2 survivor is a
...
- **6d push:** `git push` (`-u origin <branch>` if no upstream; never force-push without explicit request).
  **Confirm the commit is actually on the remote** — `git rev-parse origin/<branch>` contains
  ...
  unconfirmed → `PUSH_OK=false`, which gates EVERY commit-dependent PR mutation** — 6e `fixed:`
  replies, **6e.1 dismissals**, 6f resolves, **6h re-request**, and the report's **PR-body refresh**.
  None of them may run: the remote still lacks the fixes, so a `fixed:` reply makes the next run skip
  real work, and dismissing the blocking review makes an unfixed PR look merge-ready. Emit only the
  report's `PUSH FAILED` lead + the retry block. Capture CI URL: `CI_RUN_URL=$(gh run list --branch "$BRANCH" --limit 1 --json url --jq '.[0].url // ""')` (empty ok).
