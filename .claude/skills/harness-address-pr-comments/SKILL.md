...
`fixed:` replies by **any author with write access** (`author_association` ∈ `OWNER`/`MEMBER`/`COLLABORATOR`) **whose cited sha is an ancestor of the PR branch** — the trailer alone is unauthenticated text, so an outside commenter could otherwise post two fake `commit:<sha7>` trailers and force the next real run into the 5c.1 brake, or spoof `region_map` into false repeat-patch forks. Write-access scoping keeps the team case (teammates run it from different accounts) while excluding drive-by comments; only `fixed:` — an `already:` sha or the literal `pre-existing` isn't a patch by this
