...
- **start:** `▶ harness:status` + any target (e.g. ` · <change>`).
- **end:** `■ harness:status v<hash8> → <outcome>` — one-line result (e.g. `web-core @ ship` / `3 changes in flight`). `<hash8>` = first 8 chars of `git hash-object` on this SKILL.md — compute it (run the command) in the end-of-run commands; never a placeholder.
...
| review | the **applicable** review left its artifact — **full:** build runs **both** (`architecture-review.md` + `design-review.md`), **architecture-only** (schema-only change), or **none** (pure docs/rename, i.e. no code/spec deltas); **spec-less:** the one `spec-less-review.md`. ✓ when the applicable artifact(s) exist **or** the change type warrants no review. **Existence alone isn't proof — read each artifact's `Outcome:` line: `Outcome: not-run` means that gate failed and judged nothing (its auditor returned no usable payload), so the stage is ❌ *not* ✓, labeled "review did not run — re-run it". A not-run artifact is a record of absence, and counting it as coverage hides exactly the pipeline gap it was written to disclose.** **Never** infer review from `pr-body.md` / tasks — those are verify/ship proof, not review proof (a change can reach verify with reviews wrongly skipped) |
