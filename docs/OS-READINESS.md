...
- **2026-07-06 — public-release decision (supersedes issue #5's "curated snapshot" plan):** publish
  ...
  client/employer confidentiality surface, and negligible security value (a path would expose only the
  already-public `acatl` handle). The one illustrative path quoted in row #4 was genericized anyway for
  tidiness. Full sweep found no secrets/tokens/PII, `private: true` blocks npm
