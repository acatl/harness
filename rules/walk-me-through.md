...
is **the** path for that case — a skill that can stop must route it here. **Precedence: this outranks
"< 2 → state the call and do it" — the gate is BLOCKING.** Outcomes, both explicit: **yes** → apply and
record the approval; **no** → record declined, write nothing, and the underlying finding stays open
(denial rejects the repair, not the finding). Never auto-apply on silence, never re-render as a card.
