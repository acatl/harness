...
- **start:** `▶ harness:test-guide v<hash8>` followed by any target (e.g. ` · <change>`). `<hash8>` = `git hash-object` of this SKILL.md, first 8 chars. **Compute it (run the command); never emit a placeholder (`vTBD`, `<hash8>`, or a guess).**
- **end:** `■ harness:test-guide → <outcome>` — one-line result (e.g. `web-core · 3 walked · 1 fail` / `nothing to test`).
